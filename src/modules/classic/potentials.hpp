...
namespace qpp{
  ...
  public:
    ...
    typedef classical_potential<REAL,CELL> SELF;
    ...
    REAL energy(geometry<REAL,CELL> & geom,
		const neighbours_table<REAL,CELL> & ngbr,
		const std::vector<int> & active_atoms = {},
		const std::vector<int> & frozen_atoms = {},
		const std::vector<int> & core_shells = {} ) const{
    ...
    py::object py_energy(geometry<REAL,CELL> & geom,
			 const neighbours_table<REAL,CELL> & ngbr,
    ...
    static void py_export(py::module m, const char * pyname) {
      py::class_<classical_potential<REAL,CELL> >(m, pyname, py::dynamic_attr())
  ...
  template <class REAL, class CELL>
  REAL classical_potential<REAL,CELL>::too_close = 1e-8;
  ...
  class mm_calculator{
    ...
    //std::vector< std::shared_ptr<classical_potential<REAL,CELL> > > pot;
    std::vector<classical_potential<REAL,CELL>*> pot;
  ...
  public:
    ...
    void add_potential(classical_potential<REAL,CELL> & p)
    {
      pot.push_back(&p);
    }
	...
	bt.merge(p->distances());
      ...
      geom.build_types();
      neighbours_table<REAL,CELL>  ngbr(geom,bt);
      ngbr.build();
    ...
    py::object py_energy(geometry<REAL,CELL> & geom, bool do_d1e, bool do_d2e)
    {     
      if (!do_d1e){
	REAL E = 0e0;
	Eigen::Matrix<REAL, Eigen::Dynamic, Eigen::Dynamic> empty_d1e;
    ...
    static void py_export(py::module m, const char * pyname) {
      py::class_<mm_calculator<REAL,CELL> >(m, pyname)
	.def(py::init<>())
	.def("set_active_atoms",   &mm_calculator<REAL,CELL>::set_active_atoms)
	.def("set_frozen_atoms",   &mm_calculator<REAL,CELL>::set_frozen_atoms)
	.def("set_active_regions", &mm_calculator<REAL,CELL>::set_active_regions)
	.def("set_frozen_regions", &mm_calculator<REAL,CELL>::set_frozen_regions)
	.def("add_potential",      &mm_calculator<REAL,CELL>::add_potential)
	.def("energy",  &mm_calculator<REAL,CELL>::py_energy,
	     py::arg("geom"), py::arg("do_d1e") = false, py::arg("do_d2e") = false )
	.def("set_core_shells", &mm_calculator<REAL,CELL>::set_core_shells)
	;
    }
  ...
  template <class REAL, class CELL>
  class pair_potential : public classical_potential<REAL,CELL>{
  public:

    using classical_potential<REAL,CELL>::too_close;
    
    typedef pair_potential<REAL,CELL> SELF;
    
    using classical_potential<REAL,CELL>::distances;
    ...
    */
    ...
    static void py_export(py::module m, const char * pyname) {
      py::class_<SELF, classical_potential<REAL,CELL> >(m, pyname)
  ...
  template <class REAL, class CELL>
  class potential_3body : public classical_potential<REAL,CELL>{
  public:
    
    using classical_potential<REAL,CELL>::too_close;
    using classical_potential<REAL,CELL>::distances;
    typedef potential_3body<REAL,CELL> SELF;
  ...
  template <class REAL, class CELL>
  std::istream& operator>>(std::istream &is, pair_potential<REAL,CELL> &pp)
  {
    return pp.load(is);
  } 
